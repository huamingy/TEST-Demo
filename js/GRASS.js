...
function init() {
  ...
  loader.load('../models/GRASS/cs.FBX', function (fbx) {
    scene.add(fbx)
    console.log('fbx: ', fbx)
    // Tree('SugarPalm')
    // Tree('Tree01')
...
function changeLigtmap(name, img) {
  ...
  var lightmap = textureLoader.load(`../models/GRASS/` + name + `LightMap.jpg`)
  var aoMap = textureLoader.load(`../models/GRASS/` + name + `AO.jpg`)
